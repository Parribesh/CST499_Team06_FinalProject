...
function Tester() {
  ...
  const [location, setLocation] = useState({ // Stores a location object which contains users coordinates and current address.
    loaded: false,
    coordinates: { lat: "", lng: "" },
    address: "N/A",
  });
      ...
      setTimeout( () => {
        ...
        if (testNum.current>3) {
          ...
          if(sessionStorage.getItem('avgDown') != null){
            ...
            currentValues.push(Math.floor(((avgDownCal/calDataDown.current.length) + avgDownVirg/virgDataDown.current.length))/2);
          ...
          if(JSON.parse(sessionStorage.getItem('avgUp')).length > 0){
            ...
            currentValues[currentValues.length - 1] = Math.floor(((avgUpCal/calDataUp.current.length) + (avgUpVirg/virgDataUp.current.length))/2);
            sessionStorage.avgUp = JSON.stringify(currentValues); //Store value in sessionStorage

          }else{ // Else key does not exist yet. So make a new array append value and save to sessionStorage
            let currentValues = [];
            //Taking avg of both California and Virginia Upload Test Values
            currentValues.push(Math.floor(((avgUpCal/calDataUp.current.length) + (avgUpVirg/virgDataUp.current.length))/2));
            sessionStorage.avgUp = JSON.stringify(currentValues); //Store value in sessionStorage
          }
          // Show results Modal View
          handleShow()

        } else { // Test num is less than or equal to 3 so continue to Virgina tests
